  ...
  factory ProfileData.fromJson(Map<String, dynamic> json) {
    ...
    if (json.containsKey('user') && json['user'] is Map<String, dynamic>) {
      ...
      return ProfileData(
        ...
        email: user['email'] ?? '-',
        npm: user['npm'] ?? '-', 
        tempatTglLahir: '${user['birth_place'] ?? '-'}, ${user['birth_date'] ?? '-'}',
        alamat: user['address'] ?? '-', 
        jenisKelamin: user['gender'] ?? '-',
        
        totalSubmit: quizStats?['total_submissions'] ?? 0,
        rataRataNilai: (quizStats?['average_score'] ?? 0).toDouble(),
      );
    }
    return ProfileData(
      ...
      jenisKelamin: json['jenis_kelamin'] ?? json['gender'] ?? json['sex'] ?? '-',
      totalSubmit: json['total_submit'] ?? json['total_quiz'] ?? json['quiz_count'] ?? 0,
